...
class Curve_Binding_Tool_OT_Bind(bpy.types.Operator):
    ...
    def execute(self,context): 
        ...
        follow_Path = Constraint.FOLLOW_PATH.Add_To_Obj(empty_obj)
        Constraint.FOLLOW_PATH.Use_Fixed_Location_Set(follow_Path,True)

        #根据骨骼段数，计算曲线等分点位置
        def __Calculate_Bone_Points(curverObj,boneCount):
            #设置空物体的跟随路径目标为某根曲线
            Constraint.FOLLOW_PATH.Target_Set(follow_Path,curverObj)
            #计算几个顶点的位置系数
            points = [0]
            v = 1.0 / boneCount
            ...
            for i in range(len(points)):
                Constraint.FOLLOW_PATH.Offset_Factor_Set(follow_Path,points[i])
                Obj.Update()
