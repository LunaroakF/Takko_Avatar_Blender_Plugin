...
# 根据修改器名称获取修改器
def Get_By_Name(obj, modifier_name):
...
class __MODIFIER__:
    @classmethod
    def Create(cls,obj):
        mo = obj.modifiers.new(cls.__name__.capitalize(),cls.__name__)
        return mo.name
#精简
class DECIMATE:
    def Create(obj):
...
class ARMATURE(__MODIFIER__):
    def Object_Set(meshObj,modifierName,object):
        meshObj.modifiers[modifierName].object = object
    def Object_Get(meshObj,modifierName):
        return meshObj.modifiers[modifierName].object
