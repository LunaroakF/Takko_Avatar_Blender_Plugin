...
class Shape_Key_Tool_OT_Apply_To_Basis(bpy.types.Operator):
    bl_idname = "shape_key_tool.apply_to_basis"
    bl_label = "应用到基型"
    bl_description = "将形态键应用到基型"
    ...
    def execute(self,context): 
        ...
        #活动形态键编号
        index = ShapeKey.Active_Index_Get(meshObj)
        if index == 0: return Log.Error_Cancelled(self,"不可应用基型本身") 
        ...
        #获取形态键的位置信息
        shapeKey_position = ShapeKey.Position_Get(meshObj,index)

        #应用
        ShapeKey.Position_Set(meshObj,0,shapeKey_position)
...
class Shape_Key_Tool_OT_Mix(bpy.types.Operator):
    ...
    def execute(self,context): 
        #获取名称
        shapeKeyName = context.scene.shape_key_tool_shape_key_name
        ...
        #清空名称
        context.scene.shape_key_tool_shape_key_name = ""
